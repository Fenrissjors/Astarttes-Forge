const APP_VERSION = '2.6.0-dark-angels-module';
...
const VERIFICATION_GROUPS = CHAPTER_LIBRARY?.verificationGroups || {
  'Generic Adeptus Astartes': ['Gladius Task Force','Anvil Siege Force','Firestorm Assault Force','Ironstorm Spearhead','Stormlance Task Force','Vanguard Spearhead','First Company Task Force','Librarius Conclave','Fulguris Task Force','Subversion Assets'],
...
function runCurrentVerification(){
  ...
  if(!key || !VERIFICATION_DETACHMENTS.some(n=>verificationKey(n)===key)){ alert('Import one of the supported Generic Astartes, Space Wolves, Blood Angels or Dark Angels test rosters first.'); return; }
...
function renderVerificationDashboard(){
  ...
  } else current.innerHTML+=`<p class="muted">Import one of the supported Generic Astartes, Space Wolves, Blood Angels or Dark Angels rosters and press <strong>Run verification</strong>.</p>`;
...
function analyseImportedRoster(imported,fileName=''){
  ...
  const allRules=detachmentRows.length>0&&detachmentRows.every(r=>ruleExpectationSatisfied(r.loadedRules,r.expectation?.detachmentRules) && detachmentStructureCounts(r.merged).withBody===r.loadedRules);
...
function parseDetachmentRuleSections(text='') {
  const clean=cleanCodexText(text);
  if(!clean) return {body:'', restrictions:[], subRules:[]};
  ...
  for(const line of bodyLines){
    ...
    if(!trimmed){ prose.push(''); continue; }
    const bullet=trimmed.match(/^(?:—|-|•|▪|◦|■)\s*(.+)$/);
    if(bullet){ subRules.push({name:'',text:bullet[1].trim()}); continue; }
    const labelled=trimmed.match(/^([A-Z][A-Z0-9 '’\-]{2,40})\s*:\s*(.+)$/);
    if(labelled && !/^(WHEN|TARGET|EFFECT|DURATION|RESTRICTIONS?)$/i.test(labelled[1])){
      subRules.push({name:labelled[1].trim(),text:labelled[2].trim()});
      continue;
    }
    prose.push(trimmed);
  }
  return {body:prose.join('\n').replace(/\n{3,}/g,'\n\n').trim(),restrictions,subRules};
}
function detachmentRuleStructure(rule={}) {
  ...
  return {
    ...
    subRules:Array.isArray(parsed.subRules)?parsed.subRules:[]
  };
}
function formatDetachmentRuleText(rule={}) {
  const structure=detachmentRuleStructure(rule);
  const bodyHtml=structure.body ? `<div class="detachment-rule-body">${structure.body.split(/\n\n+/).map(p=>`<p>${escapeHtml(p).replace(/\n/g,'<br>')}</p>`).join('')}</div>` : '';
  const subHtml=structure.subRules.length ? `<div class="detachment-subrules">${structure.subRules.map(s=>`<div class="detachment-subrule">${s.name?`<strong>${escapeHtml(s.name)}</strong>`:''}<p>${escapeHtml(s.text)}</p></div>`).join('')}</div>` : '';
  const restrictionsHtml=structure.restrictions.length ? `<div class="detachment-restrictions"><strong>RESTRICTIONS</strong>${structure.restrictions.map(r=>`<p>${escapeHtml(r).replace(/\n/g,'<br>')}</p>`).join('')}</div>` : '';
  return `<div class="official-rule-text detachment-rule-text">${bodyHtml}${subHtml}${restrictionsHtml}</div>`;
}
...
function renderSourceEditionInspector(){
  ...
  host.innerHTML=`
    ...
    <div class="source-flow"><span>ROSZ</span><b>→</b><span>${escapeHtml(edition.schemaId||'unknown')} schema</span><b>→</b><span>${allUnits().length} normalized units</span><b>→</b><span>${fallbackCount} library fallback${fallbackCount===1?'':'s'}</span><b>→</b><span>Print renderer</span></div>
...
function buildLosslessRosterSourceGraph(doc) {
  ...
  return {version:1,selections,profiles,rules,categories,costs};
}
...
function normalizeArmyFromSourceGraph(sourceGraph, sourceName='roster.ros') {
  ...
  unitRoots.forEach((root,index)=>{
    ...
    allProfiles.filter(p=>!weaponProfiles.includes(p) && !/^(unit|model)$/i.test(p.type)).forEach(p=>{
      ...
      const owner=selectionById.get(p.ownerSelectionId)||root; const modelOwner=nearestModel(owner,root);
      abilityItems.push({sourceProfileId:p.id,model:modelOwner.name||root.name,name:p.name||p.type||'Ability',text});
    });
    ...
    const ruleItems=allRules.filter(r=>!weaponOwnerIds.has(r.ownerSelectionId) && r.name).map(r=>{
      ...
      return {sourceRuleId:r.id,model:modelOwner.name||root.name,name:normaliseRuleName(r.name)};
    });
    ...
    }).map(sel=>({name:sel.name,sourceSelectionId:sel.id,points:(costsByOwner.get(sel.id)||[]).filter(c=>/pts|point/i.test(c.name)).reduce((sum,c)=>sum+c.value,0)}));
...
function exactDetachmentRulesFromSelection(detachmentInfo){
  ...
  const pushRule=(name,text,sourceType,sourceId='')=>{
    ...
    if(!name||!text) return;
    found.push({kind:'detachment',name,text,sections:parseDetachmentRuleSections(text),sourceType,sourceId,sourcePath:selectionPath(node)});
  };
...
function parseRosterXml(xmlText, sourceName='roster.ros') {
  ...
  const detachmentsData=detachmentInfos.map(info=>{
    ...
    const raw={
      id:info.id,name:info.name,dp:info.dp,
  ...
  return {name,source:sourceName,units:normalized.units,entries:normalized.entries,rules:uniqueRules,points,detachment:detachmentInfo.name,detachments:detachmentInfos.map(d=>d.name),detachmentId:detachmentInfo.id,faction,catalogue,importGraph,sourceInspection,detachmentData,detachmentsData};
}
