...
ok(app.includes('function canUseDarkAngelsDatasheetPrototype'),'Dark Angels prototype route exists');
ok(app.includes("chapterKey!=='dark-angels'"),'prototype is restricted to Dark Angels');
...
ok(app.includes("fragmentCardElement(createCard(entry,unit,'print'))"),'prototype reuses real print datasheet components');
...
ok(app.includes('fitAllAdaptiveArtworkPages(output);'),'printed output receives measured fit before printing');
...
ok(css.includes('font-size:var(--a4-desc-font,12pt)!important'),'Dark Angels description typography is explicitly controlled');
if(!process.exitCode) console.log('Dark Angels explicit pixel-fit verification complete.');
